...
const ProductForm = ({ initialData = {}, onSubmit, fetchProduct }) => {
    ...
    const [price, setPrice] = useState(initialData.price || '');
    const [stock, setStock] = useState(initialData.stock || '');
    const [category, setCategory] = useState(initialData.category || ''); // Додано стан для категорії
    const [images, setImages] = useState([]);
    const [imagePreviews, setImagePreviews] = useState(initialData.images ? initialData.images.map(image => `${baseUrl}${image}`) : []);
    const [errors, setErrors] = useState('');
    ...
    useEffect(() => {
        ...
        setPrice(initialData.price || '');
        setStock(initialData.stock || '');
        setCategory(initialData.category || ''); // Ініціалізація категорії
        setImagePreviews(initialData.images ? initialData.images.map(image => `${baseUrl}${image}`) : []);
    }, [initialData]);
    ...
    const handleSubmit = (e) => {
        ...
        productData.append('price', price);
        productData.append('stock', stock);
        productData.append('category', category); // Додано категорію
        images.forEach((image, index) => {
            productData.append(`images`, image);
        });
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <div className={styles.formGroup}>
                <select
                    ...
                    required
                >
                    <option className={styles.categoryHeader} value="" disabled>Виберіть категорію</option>

                    <option className={styles.categoryHeader} value="" disabled>Одяг</option>
                    <option className={styles.optionActive} value="Чоловічий одяг">Чоловічий одяг</option>
                    <option value="Жіночий одяг">Жіночий одяг</option>
                    <option value="Детячий одяг">Детячий одяг</option>

                    <option className={styles.categoryHeader} value="" disabled>Аксесуари</option>
                    <option value="Годинники">Годинники</option>
                    <option value="Прикраси">Прикраси</option>
                    <option value="Сумки">Сумки</option>

                    <option className={styles.categoryHeader} value="" disabled>Сувеніри</option>
                    <option value="Магніти">Магніти</option>
                    <option value="Брелки">Брелки</option>
                    <option value="Статуетки">Статуетки</option>

                    <option className={styles.categoryHeader} value="" disabled>Канцелярія</option>
                    <option value="Ручки">Ручки</option>
                    <option value="Блокноти">Блокноти</option>
                    <option value="Щоденники">Щоденники</option>

                    <option className={styles.categoryHeader} value="" disabled>Весь каталог</option>
